...
### .pnpm-install-done が残る問題

- 症状: コンテナやローカルで依存インストール完了後に `.pnpm-install-done` ファイルがリポジトリに残り続ける。
- 原因: インストーラコンテナが完了フラグを生成するが、削除する処理が実装されていない。
- 対処案: インストール完了フラグをリポジトリに含めない運用に変更し、インストーラ側でコンテナ終了時にフラグを削除するスクリプトを実行する。

### shared の型共有 — backend 側で class を使うべき箇所に interface を使用している問題

- 問題: `shared` パッケージでエクスポートしている型が `interface` のみになっており、バックエンド側で `class-transformer` / `class-validator` や DI によるインスタンス生成を利用する箇所で必要なランタイムの型情報やデコレータが使えない。
- 対処案: バックエンド互換が必要な型は `class`（必要に応じてデコレータを付与）として `shared` に置くか、`shared/types`（interface）と `shared/entities`（class）などに分離して用途ごとに使い分ける。
- 推奨: フロントエンド向けの純粋な型宣言は `types` に保持し、バックエンドでのバリデーションやシリアライズが必要なエンティティは `class` として `shared` に追加することを推奨する。
